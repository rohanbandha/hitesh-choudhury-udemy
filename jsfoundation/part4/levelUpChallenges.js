...
1. Write a `for` loop that loops through the array `["green tea", "black tea", "chai" ,"oolong tea"]` and stops the loop when it finds `"chai"`.
   Store all tea before `"chai"` in a nee array named `selectedTeas`.
*/
...
let preferredTeas = []
for (const teea of teaTypes) {
    if (teea === "herbal tea") {
        continue;
    }
    preferredTeas.push(teea)
}
...
teaCollection.forEach(teeea => {
    if (teeea === "chai") {
        return;
    }
    availableTeas.push(teeea);
});
